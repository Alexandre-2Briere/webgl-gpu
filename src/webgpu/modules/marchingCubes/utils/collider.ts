...
export class TriangleSpatialHash {
    ...
    private readonly offset: Vertex3;

    constructor(result: MarchingCubesResult, worldOffset: Vertex3) {
    ...
    collidesWithAABB(
        ...
        const localMinX = worldMinX - this.offset.x;
        const localMinY = worldMinY - this.offset.y;
        const localMinZ = worldMinZ - this.offset.z;
        const localMaxX = worldMaxX - this.offset.x;
        const localMaxY = worldMaxY - this.offset.y;
        const localMaxZ = worldMaxZ - this.offset.z;
        ...
        const { x: offX, y: offY, z: offZ } = this.offset;
    ...
    /**
     * Returns the outward unit normal of the first triangle in this chunk's mesh
     * that overlaps the given world-space AABB, or null if there is no overlap.
     * "Outward" is defined as ny >= 0 (normal pointing generally upward), which
     * is reliable for terrain surfaces regardless of marching-cubes winding order.
     */
            ...
            for (let iy = iy0; iy <= iy1; iy++) {
                for (let iz = iz0; iz <= iz1; iz++) {
                    ...
                    for (let j = 0; j < cell.length; j++) {
                        ...
                        const b = t * 9;
                        const ax = v[b]     + offX, ay = v[b + 1] + offY, az = v[b + 2] + offZ;
                        const bx = v[b + 3] + offX, by = v[b + 4] + offY, bz = v[b + 5] + offZ;
                        const cx = v[b + 6] + offX, cy = v[b + 7] + offY, cz = v[b + 8] + offZ;

                        if (!aabbTriangleOverlap(cX, cY, cZ, hX, hY, hZ, ax, ay, az, bx, by, bz, cx, cy, cz)) continue;
                        ...
                        if (len < 1e-8) continue; // degenerate triangle, skip

                        nx /= len; ny /= len; nz /= len;
                        // Flip so the normal always points generally upward.
                        // This makes the result consistent regardless of winding order.
    ...
    raycastTriangle(
        ...
        const lox = ox - this.offset.x;
        const loy = oy - this.offset.y;
        const loz = oz - this.offset.z;
