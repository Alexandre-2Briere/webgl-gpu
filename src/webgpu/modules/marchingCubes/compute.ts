...
import { CORNER_OFFSETS, EDGE_CORNERS } from '../../common/constants/constants';
    ...
    for (let z = 0; z < depth - 1; z++) {
        for (let y = 0; y < height - 1; y++) {
            for (let x = 0; x < width - 1; x++) {
                ...
                // WHY field.xzStep / field.yStep: XZ uses a finer grid spacing
                // than Y, so corner positions must be scaled per axis.
