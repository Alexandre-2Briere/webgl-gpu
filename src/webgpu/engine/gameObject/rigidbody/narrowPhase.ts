import type { Hitbox3D, SphereHitbox, CubeHitbox, CapsuleHitbox, MeshHitbox, PlaneHitbox } from '../hitbox';
import { type CollisionResult, NO_HIT, flipNormal } from './narrowPhaseHelper/helpers';
...
import { testPlaneSphere, testPlaneCube, testPlaneCapsule, testPlaneMesh, testPlanePlane } from './narrowPhaseHelper/planeTests';
...
export function narrowPhase(a: Hitbox3D, b: Hitbox3D): CollisionResult {
  ...
  if (typeA === 'sphere') {
    ...
    if (typeB === 'plane')   return flipNormal(testPlaneSphere(b as PlaneHitbox, sa));
  }
  ...
  if (typeA === 'cube') {
    ...
    if (typeB === 'plane')   return flipNormal(testPlaneCube(b as PlaneHitbox, ca));
  }
  ...
  if (typeA === 'capsule') {
    ...
    if (typeB === 'plane')   return flipNormal(testPlaneCapsule(b as PlaneHitbox, ca));
  }
  ...
  if (typeA === 'mesh') {
    ...
    if (typeB === 'plane')   return flipNormal(testPlaneMesh(b as PlaneHitbox, ma));
  }

  if (typeA === 'plane') {
    const pa = a as PlaneHitbox;
    if (typeB === 'sphere')  return testPlaneSphere(pa, b as SphereHitbox);
    if (typeB === 'cube')    return testPlaneCube(pa, b as CubeHitbox);
    if (typeB === 'capsule') return testPlaneCapsule(pa, b as CapsuleHitbox);
    if (typeB === 'mesh')    return testPlaneMesh(pa, b as MeshHitbox);
    if (typeB === 'plane')   return testPlanePlane();
  }
