...
import type { Quad2DOptions, Quad2DHandle } from '../types'
...
export class Quad2D implements Renderable, Quad2DHandle {
  readonly id = Symbol()
  ...
  private _uniformData = new Float32Array(20)  // 16 (identity model) + 4 (tint)

  constructor(opts: Quad2DOptions) {
    this._opts = opts
    // Identity model matrix + tint from color
  ...
  setRect(x: number, y: number, width: number, height: number): void {
    ...
    this._vertexBuf.write(this._buildVerts(this._opts))
  }

  private _buildVerts(o: Quad2DOptions): Float32Array {
    const { x, y, width, height, color: [r, g, b, a] } = o
