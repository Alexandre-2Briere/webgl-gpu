...
import type { SaveSegments, GameObjectSnapshot, LightObjectSnapshot } from '../../saveManager/types';
...
function makeSegments(
  ...
  lightObjects: LightObjectSnapshot[] = [],
): SaveSegments {
  return {
    sceneConstants: [{ version: 1, camera: CAMERA_FIXTURE }],
    gameObjects:    [{ version: 1, objects: gameObjects }],
    lightObjects:   [{ version: 1, objects: lightObjects }],
  };
}
...
describe('restoreFromSnapshot — camera', () => {
  it('creates a camera with position/yaw/pitch from sceneConstants', async () => {
    await restoreFromSnapshot(engine, makeSegments());
  ...
  it('skips camera setup when sceneConstants is empty', async () => {
    const segments: SaveSegments = {
      sceneConstants: [],
      gameObjects:    [{ version: 1, objects: [] }],
      lightObjects:   [{ version: 1, objects: [] }],
    };
    await restoreFromSnapshot(engine, segments);
    expect(engine.createCamera).not.toHaveBeenCalled();
    expect(engine.setCamera).not.toHaveBeenCalled();
  });
});
...
describe('restoreFromSnapshot — Cube', () => {
  it('calls createMesh for a Cube record', async () => {
    await restoreFromSnapshot(engine, makeSegments([{ key: 'Cube', ...BASE_OBJECT }]));
  ...
  it('passes color from the snapshot into the cube vertices', async () => {
    ...
    await restoreFromSnapshot(engine, makeSegments([{ key: 'Cube', ...BASE_OBJECT, color }]));
  ...
  it('passes position, quaternion, and scale', async () => {
    ...
    await restoreFromSnapshot(engine, makeSegments([{ key: 'Cube', ...BASE_OBJECT, position, quaternion, scale }]));
    const call = (engine.createMesh as ReturnType<typeof vi.fn>).mock.calls[0][0];
...
describe('restoreFromSnapshot — Quad', () => {
  it('calls createQuad3D for a Quad record', async () => {
    await restoreFromSnapshot(engine, makeSegments([{ key: 'Quad', ...BASE_OBJECT }]));
  ...
  it('maps scale[0] to width and scale[2] to height', async () => {
    ...
    await restoreFromSnapshot(engine, makeSegments([{ key: 'Quad', ...BASE_OBJECT, scale }]));
  ...
  it('passes color into the quad renderable', async () => {
    ...
    await restoreFromSnapshot(engine, makeSegments([{ key: 'Quad', ...BASE_OBJECT, color }]));
    const call = (engine.createQuad3D as ReturnType<typeof vi.fn>).mock.calls[0][0];
...
describe('restoreFromSnapshot — FBX', () => {
  ...
  it('calls loadFbx with the assetUrl', async () => {
    await restoreFromSnapshot(engine, makeSegments([fbxRecord]));
  ...
  it('calls loadFbx only once when two records share the same URL', async () => {
    await restoreFromSnapshot(engine, makeSegments([fbxRecord, fbxRecord]));
  ...
  it('calls loadFbx twice for two records with different URLs', async () => {
    ...
    await restoreFromSnapshot(engine, makeSegments([fbxRecord, second]));
...
describe('restoreFromSnapshot — lights', () => {
  ...
  it('calls createPointLight for lightType 1', async () => {
    await restoreFromSnapshot(engine, makeSegments([], [{ key: 'Light', lightType: 1, ...BASE_LIGHT }]));
    expect(engine.createPointLight).toHaveBeenCalledWith({ radius: 5 });
  });

  it('calls createAmbientLight for lightType 0', async () => {
    await restoreFromSnapshot(engine, makeSegments([], [{ key: 'Light', lightType: 0, ...BASE_LIGHT }]));
    expect(engine.createAmbientLight).toHaveBeenCalledOnce();
  });

  it('calls createDirectionalLight for DirectionalLight key', async () => {
    await restoreFromSnapshot(engine, makeSegments([], [{ key: 'DirectionalLight', lightType: 2, ...BASE_LIGHT }]));
  ...
  it('calls setPosition on the light with the record position', async () => {
    ...
    await restoreFromSnapshot(engine, makeSegments([], [{ key: 'Light', lightType: 1, ...BASE_LIGHT, position }]));
  ...
  it('calls setColor on the light with the record color', async () => {
    ...
    await restoreFromSnapshot(engine, makeSegments([], [{ key: 'Light', lightType: 1, ...BASE_LIGHT, color }]));
    const light = (engine.createPointLight as ReturnType<typeof vi.fn>).mock.results[0].value;
  ...
  it('calls setDirection on the light', async () => {
    ...
    await restoreFromSnapshot(engine, makeSegments([], [{ key: 'DirectionalLight', lightType: 2, ...BASE_LIGHT, direction }]));
...
describe('restoreFromSnapshot — physics', () => {
  it('calls onFrame with a function when at least one object has a rigidbody', async () => {
    ...
    await restoreFromSnapshot(engine, makeSegments([{ key: 'Cube', ...BASE_OBJECT, physicsConfig }]));
...
// ── Multiple segment entries ──────────────────────────────────────────────────

describe('restoreFromSnapshot — multiple segment entries', () => {
  it('spawns objects from all gameObjects entries', async () => {
    const segments: SaveSegments = {
      sceneConstants: [{ version: 1, camera: CAMERA_FIXTURE }],
      gameObjects: [
        { version: 1, objects: [{ key: 'Cube', ...BASE_OBJECT }] },
        { version: 1, objects: [{ key: 'Quad', ...BASE_OBJECT }] },
      ],
      lightObjects: [{ version: 1, objects: [] }],
    };
    await restoreFromSnapshot(engine, segments);
  ...
  it('spawns lights from all lightObjects entries', async () => {
    const BASE_LIGHT = { ...BASE_OBJECT, radius: 5, direction: [0, -1, 0] as [number, number, number] };
    const segments: SaveSegments = {
      sceneConstants: [{ version: 1, camera: CAMERA_FIXTURE }],
      gameObjects:    [{ version: 1, objects: [] }],
      lightObjects: [
        { version: 1, objects: [{ key: 'Light', lightType: 1, ...BASE_LIGHT }] },
        { version: 1, objects: [{ key: 'DirectionalLight', lightType: 2, ...BASE_LIGHT }] },
      ],
    };
    await restoreFromSnapshot(engine, segments);
    expect(engine.createPointLight).toHaveBeenCalledOnce();
    expect(engine.createDirectionalLight).toHaveBeenCalledOnce();
  });
});
